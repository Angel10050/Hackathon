...
let preciosDeRobots = boton.forEach(el => el.addEventListener('click',function anadiendoPreciosDeRobots (){
    arrFlag.push(el.value)

    let sumaDeCompra = arrFlag.reduce(function(PrevVal, currentVal){
        return PrevVal + currentVal
    },0)
    
    
    let miCarrito = document.querySelector('.cart-total-quantity')
    miCarrito.innerHTML = sumaDeCompra
console.log(arrFlag)
}))
